...
import type { ContaReceberResponse, StatusConta } from "@/types/financeiro";
...
export function ContasReceberPage() {
  ...
  const [dados, setDados] = useState<PaginaResponse<ContaReceberResponse> | null>(null);
  ...
  async function carregar() {
    ...
    try {
      const resposta = await contaReceberApi.listar({
      ...
      setErro(extrairMensagemErro(err, "Não deu pra carregar as contas a receber"));
    } finally {
      setCarregando(false);
    }
  }
  ...
  async function handleMarcarRecebida(id: string) {
    ...
    try {
      await contaReceberApi.marcarComoRecebida(id);
      carregar();
    } catch (err) {
      setErro(extrairMensagemErro(err, "Não deu pra marcar essa conta como recebida"));
    } finally {
  ...
  async function handleCancelar(id: string) {
    ...
    setAcaoEmAndamento(id);
    try {
      await contaReceberApi.cancelar(id);
      carregar();
    } catch (err) {
    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="mx-auto max-w-4xl px-6 py-12">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-4">
          ...
          <Link to="/financeiro/contas-a-receber/nova">
                ...
                <tbody>
                  {dados?.content.map((conta) => (
                    <tr key={conta.id} className="border-b border-ink-100 last:border-0">
                      <td className="px-4 py-3 font-medium text-ink">{conta.descricao}</td>
                      <td className="px-4 py-3 text-ink-400">{conta.clienteNome || "—"}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-3">
                          {conta.status === "ABERTA" && (
                            <>
                              <Link to={`/financeiro/contas-a-receber/${conta.id}/editar`} className="text-xs font-medium text-ink-400 hover:text-ink">
                                Editar
                              </Link>
                              <button
